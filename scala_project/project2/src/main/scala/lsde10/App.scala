...
object App {
  ...
  def main(args : Array[String]) {
	...
	val allShipsLocation = decoded.map(p => ((p._2.getUserId(),p._1.substring(0,p._1.indexOf(".")).toLong), p._2.getPos().getGeoLocation()))
							.filter(p => if(p._2 == null) false else true)
	decoded.unpersist()
	...
	var result = allAreas.join(nonTransmittingAreas)
							.map(p => (p._1,(100*p._2._2.toFloat)/p._2._1.toFloat))
	allAreas.unpersist()
