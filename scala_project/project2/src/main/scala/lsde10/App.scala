...
object App {
  ...
  def main(args : Array[String]) {
	...
	//Compute the outages
	//data4 = ((mmsi,timestamp), GeoLocation) 
	val data4 = decoded.map(p => ((p._2.getUserId(),p._1.substring(0,p._1.indexOf(".")).toInt), p._2.asInstanceOf[AisPositionMessage].getPos().getGeoLocation()))
	
	//Check if GeoLocation is null
	val data5 = data4.filter(p => if(p._2 == null) false else true)
	...
	//slide over the data to get the gaps, the resulting type is (mmsi,gap,lat,long) both latitude and longitude are truncated to reflect the "area"
	var data = sorted.sliding(2).collect({case Array((key1, val1), (key2, val2)) if key1 == key2 => (key1._1, key2._2 - key1._2, (math floor val1.getLatitude() *10)/10, (math floor val1.getLongitude() *10)/10)})
	data.cache()
	...
	//Compute the count of ships that send in a given area
